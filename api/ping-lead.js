...
//   const LP_CAMPAIGN_ID = '689103b4b9e62';
//   const LP_CAMPAIGN_KEY = 'wQht6R8X7H3kTm9LqpcY';

//   const caller_id = body.caller_id || body.phone_number || '';
//   if (!caller_id) {
//     return new Response(
//       JSON.stringify({ success: false, message: 'caller_id (phone number) is required' }),
...
//     return new Response(JSON.stringify(resp.data), { status: 200, headers: corsHeaders() });
//   } catch (err) {
//     return new Response(
//       JSON.stringify({ success: false, message: err.response?.data || err.message }),
...
// // OPTIONS method to handle preflight
...
export async function POST(req) {
  ...
  // caller_id is strictly required (no fallback)
  const caller_id = body.caller_id || '';
  if (!caller_id) {
    return new Response(
      JSON.stringify({ success: false, message: 'caller_id is required' }),
      { status: 400, headers: corsHeaders() }
    );
  }
  ...
  // Optional fields from documentation (in correct order)
  const optionalFields = [
    's1', 's2', 's3', 's4', 's5',
    'first_name', 'last_name', 'phone_number', 'email_address',
    'address', 'city', 'state', 'zip_code', 'dob', 'ip_address',
    'gender', 'marital_status', 'jornaya_lead_id', 'trusted_form_cert_id'
  ];
