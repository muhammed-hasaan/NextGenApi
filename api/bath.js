...
//             'FirstName', 'LastName', 'Phone',
//             'Address', 'City', 'State', 'Zip',
//             'jobType', 'spaceType', 'propertyType', 'occupancy', 'caller_id'
//         ];
...
//                 console.log('External API Response:', postResponse); // Log the external API response
...
export default async function handler(req, res) {
    ...
    try {
        ...
        // Determine post type from query parameters
        const postType = (req.query.postType || 'simple').toLowerCase();
        ...
        // Add ClickID to required fields if postType is secure
        if (postType === 'secure') {
            requiredFields.push('ClickID');
        }
        ...
        // Determine post URL based on post type
        const postUrl = postType === 'secure' ? SECURE_POST_URL : SIMPLE_POST_URL;
        ...
        // Parse the external API response to extract lead_id
        let leadId = null;
        try {
            const responseData = JSON.parse(postResponse.data);
            // Adjust the key based on the actual response structure
            leadId = responseData.leadId || responseData.id || responseData.LeadId; // Try different possible keys
        } catch (e) {
